...
Objetivo: Escrever diversas funções para reaproveitar trechos de código Dificuldade: Intermediário
Escreva um código de modo que exiba o valor do x digitado pelo usuário e que seja substituído nas funções.
...
6 - Considere as seguintes funções: f(x) = x - 4 e g(x) = 5x + 1. Qual é o valor da função composta g(f(3))? Depois crie
um código que retorne o valor da soma das duas funções com números digitados pelo usuário.
7 - Crie uma função chamada dado() que retorna, através de sorteio, um número de 1 até 6.
Exiba 10 números sorteados utilizando a mesma função criada. Números aleatórios: random.randint(inicio, fim)
"""